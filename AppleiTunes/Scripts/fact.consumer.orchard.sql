...
-- This will load semantic tables for AppleiTunes Orchard Downloads
-- In prod-main DB
-- Step 1 of 1
--------------------------------------------------- INSTRUCTIONS END -----------------------------------
insert into ca_admin.itune_job_logs (job_name,step_name) values ('itunes_dl_orchard','loading started');

insert into ca_admin.itune_job_logs (job_name,step_name) values ('itunes_dl_orchard','cleanup raw tables'); 
BEGIN;


update datacollector_db.itunes_downloads_orchard
...
;
COMMIT;
analyze datacollector_db.itunes_downloads_orchard;

-- ca.dim_itunes_partner_info
insert into ca_admin.itune_job_logs (job_name,step_name) values ('itunes_dl_orchard','1:load dim_itunes_partner_info');
BEGIN;
...
from etl_stage.vw_itunes_load_dc_orchard
...
insert into ca_admin.itune_job_logs (job_name,step_name) values ('itunes_dl_orchard','1:load dim_itunes_promo_info');
BEGIN;
insert into ca.dim_itunes_promo_info 
(promo_code,cma_code,etl_hash)
select A.promo_code,A.cma_code,A.hash_promo_key 
from etl_stage.vw_itunes_load_dc_orchard A
...
insert into ca_admin.itune_job_logs (job_name,step_name) values ('itunes_dl_orchard','1:load dim_itunes_partner_regions');
BEGIN;
...
-- 
insert into ca_admin.itune_job_logs (job_name,step_name) values ('itunes_dl_orchard','1:load dc_dim_apple_consumers');

BEGIN;
-- 1. create new consumers into mapping table
insert into etl_stage.dc_dim_apple_consumers_orchard 
(consumer_id)
select consumer_id
from datacollector_db.itunes_downloads_orchard A
group by 1
minus
(select A.consumer_id
from ca.dim_amusic_consumers_orchard A
group by 1 
union all
select A.consumer_id
from etl_stage.dc_dim_apple_consumers_orchard  A
group by 1
)
;
analyze etl_stage.dc_dim_apple_consumers_orchard;
COMMIT;

BEGIN;
-- 2. load into temp table
truncate table etl_stage.stg_dc_dim_apple_consumers_orchard_temp;
insert into etl_stage.stg_dc_dim_apple_consumers_orchard_temp
(consumer_id,consumer_key)
select A.consumer_id,min(joined.consumer_key )as consumer_key
	from datacollector_db.itunes_downloads_orchard A,
	(
		select consumer_id, min(consumer_key) as consumer_key
		from ca.dim_amusic_consumers_orchard AA 
		group by 1
		union
		select consumer_id, min(consumer_key) as consumer_key
		from etl_stage.dc_dim_apple_consumers_orchard BB 
		group by 1 
	) joined
...
(
	...
	from etl_stage.stg_dc_dim_apple_consumers_orchard_temp A
) DD 
...
insert into ca_admin.itune_job_logs (job_name,step_name) values ('itunes_dl_orchard','1:loading dim_amusic_consumers_orchard');

-- load consumers downloads
insert into ca.dim_amusic_consumers_orchard
(consumer_key,consumer_id)
select consumer_key,consumer_id
from etl_stage.dc_dim_apple_consumers_orchard
where consumer_id not in (select consumer_id from ca.dim_amusic_consumers_orchard)
;
analyze ca.dim_amusic_consumers_orchard;


insert into ca_admin.itune_job_logs (job_name,step_name) values ('itunes_dl_orchard','1:load etl_stage.stg_itunes_dl_main_orchard');
truncate table etl_stage.stg_itunes_dl_main_orchard;
BEGIN;
insert into etl_stage.stg_itunes_dl_main_orchard 
(	
report_date_key
,download_date_key
,consumer_key
...
,hash_promo_key
from etl_stage.vw_itunes_load_dc_orchard A 
where A.report_date_key not in (select report_date_key from ca.fact_itunes_downloads_orchard group by 1)
--where (A.report_date_key,load_id) in (select report_date_key,load_id from etl_stage.stg_itunes_dl_dates_orchard group by 1,2)
;
COMMIT;

BEGIN;
--  client_key
update etl_stage.stg_itunes_dl_main_orchard 
set client_key=filter_key2
from
(select
 XX.client_key as filter_key2,
  A.raw_client_key as raw_client_key2
from etl_stage.stg_itunes_dl_main_orchard A,
...
from etl_stage.stg_itunes_dl_main_orchard  A,ca.vw_all_dst_keys XX
...
from etl_stage.stg_itunes_dl_main_orchard A, ca.vw_all_ppv_keys  XX
...
from etl_stage.stg_itunes_dl_main_orchard A, ca.vw_all_spnl_keys XX
...
from etl_stage.stg_itunes_dl_main_orchard A, ca.vw_all_sales_types XX
...
from etl_stage.stg_itunes_dl_main_orchard A, ca.vw_all_sales_divisions XX
...
from etl_stage.stg_itunes_dl_main_orchard A, ca.vw_all_sales_categories XX
...
from etl_stage.stg_itunes_dl_main_orchard A, ca.dim_itunes_partner_info XX
...
from etl_stage.stg_itunes_dl_main_orchard A, ca.dim_itunes_promo_info XX
...
from etl_stage.stg_itunes_dl_main_orchard A, ca.dim_itunes_partner_regions XX
...
;
COMMIT;
insert into ca_admin.itune_job_logs (job_name,step_name) values ('itunes_dl_orchard','1:load ca.fact_itunes_download_orchard');

BEGIN;

insert into ca.fact_itunes_downloads_orchard
(
report_date_key
,download_date_key
,consumer_key
...
from etl_stage.stg_itunes_dl_main_orchard A 
where 
A.report_date_key not in (select report_date_key from ca.fact_itunes_downloads_orchard group by 1)

;
COMMIT;
analyze ca.fact_itunes_downloads_orchard;

insert into ca_admin.itune_job_logs (job_name,step_name) values ('itunes_dl_orchard','1:loading completed');
