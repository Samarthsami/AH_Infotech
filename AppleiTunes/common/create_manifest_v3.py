...
v_procname = 'write_manifest_file'
#--------------- FUNCTION --------------------------------------
# Name:             write_manifest_file
# Purpose:          genrate the manifest file and save it to S3
# Return:           return path to the manifest file
# Author:           Brijesh Gaur
# Arguments:        load_id
# Modified date     Comments
# 20.09.2018        Initial version
#
#
#---------------------------------------------------------------

def write_manifest_file(p_load_id,p_folder,p_param_name,p_client="2222"):
    try:
      """
      if p_load_id == None:
          load_id = 0
      ...
      if p_folder.find('/') == -1:
        ...
        print("==> ",p_load_id, p_folder, p_param_name)
        source_folder_name = p_folder.split('/')[0]
        subfolder_name = p_folder.replace(p_folder.split('/')[0],'')

      print("source_folder_name:", source_folder_name) #non_consumer_files
      print("subfolder_name:", subfolder_name) #/DL_nc_ZA_14247_561007_enriched.bsv.gz
      ...
      subfolder_name=[]
      subfolder_name_1 = list(set(subfolder_name))
      for x in fileExpectation:
          userFile= x[12]

          day_folder = "report_date="+userFile.split("/")[8][0:4]+"-"+userFile.split("/")[8][4:6]+"-"+userFile.split("/")[8][6:]
          provider = x[2]
          s3_folder_manifest = s3_folder + '/' + v_client+"/" + provider + '/' + source_folder_name+"/"+ day_folder+"/"
          #subfolder_name=[]
      for obj in bucket.objects.filter(Prefix=s3_folder_manifest):
          print('obj = ',obj)
          filename = '{0}/{1}'.format(bucket.name, obj.key)
          #print("filename_1 >",filename)
          #subfolder_name=''
          if filename.find('_SUCCESS') == -1 and filename.find('manifest') == -1:
              subfolder_name.append(filename.split("/")[6])
              print("filename_1 >",filename)
                  #s3_folder_manifest = s3_folder + '/' + v_client+"/" + provider + '/' + source_folder_name+"/"+ day_folder+"/"
      print("subfolder_name >>",subfolder_name)
      subfolder_name_1 = list(set(subfolder_name))
      for i in subfolder_name_1:
          s3_folder_1 = s3_folder + '/'+v_client+"/"+ provider + '/' + source_folder_name+ '/' + day_folder +"/"+ i + '/'
          #s3_folder_1.append(s3_folder + '/'+v_client+"/"+ provider + '/' + source_folder_name+ '/' + day_folder +"/"+ i + '/')
          print("s3_folder_1 >> ",s3_folder_1) #partner_data/sme/P001/non_consumer_files/report_date=2023-10-02/DL_nc_ZA_14247_561007_enriched.bsv.gz/


          for obj in bucket.objects.filter(Prefix=s3_folder_1):
              filename = '{0}/{1}'.format(bucket.name, obj.key)
      ...
      print("manifest_file_str === ",manifest_file_str)
      print("s3_folder >",s3_folder)
      #sub_folder_name = s3_folder_1.split("/")[3]
      #print("sub_folder_name === ",sub_folder_name)
      manifest_file_name = s3_folder_manifest + provider + "_" + str(load_id) +'_' + source_folder_name + '_' + day_folder + '.manifest'
      print("manifest_file_name == >> ",manifest_file_name)
...
if __name__ == '__main__':
    ...
    print(write_manifest_file(load_id,source_folder_name,param_name,v_client))
