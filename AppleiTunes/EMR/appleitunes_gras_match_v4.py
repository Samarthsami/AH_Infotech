...
from create_manifest_v3 import *
...
def proc_appleitunes(procargs):
   ...
   #config.metadata=read_config(section="table")
   #print("procargs >>> ",procargs)
   tid = procargs[0][1]
   loadid = procargs[0][0] 
    
   
   #########################################
   sql_query = "select * from "+ config.metadata["trans_control"] +" where load_id = "+ str(loadid) + " \
   ...
   all_trackFile = '/'.join(trackFile.split("/")[:9])+'/*.txt.gz'
   tid = procargs[0][1]
   loadid = procargs[0][0]
    
   trackFile = procargs[0][12].lower()
   #print("procargs -->>>> ",procargs)
   #trackFile = procargs[0][12].lower()
   #trackFile = '/'.join(trackFile.split("/")[:9])+'/*.txt.gz'
   print("all_trackFile -->> ",all_trackFile) #s3://sme-ca-dl-dev-input/default/appleitunes/sme/downloads/v1/20240401/*.txt.gz
   #exit()
   #transid = procargs[1]
   ##############################################################
   clientKey = procargs[0][27]
   provider_key = procargs[0][2]
   report_day = trackFile.split("/")[8]
   vendor_id = trackFile.split("_")[2]
   coun_key = procargs[0][3]
   ...
   write_log_info(v_procname,trackFile,str(loadid),str(tid),provider_key)
   #exit()
  ...
  #BY IK   exec(cmd)
   ...
   #exit()
   fileDf = sqlContext.read.format("csv").option("header","true").option("delimiter","\t").option("samplingRatio", "0.1").load(all_trackFile)
   print("=================================================>>>>>>>>>>>>>>>>>>>>>")
   fileDf.count()
   #fileDf.show(5,False)
   ...
   #print("------ Done apply_special_rules >>>>>> ",tmp_intermediate_fname)
   #exit()
  #BY IK  exec('spcial_gras_Df = sqlContext.read.parquet("' + tmp_intermediate_fname +'/*")')
   ...
   write_log_info(v_procname,' special gras rules completed '+str(spcial_gras_Df.count()),str(loadid),str(tid),provider_key)
   ...
   test1 = generic_gras_Df.agg(sum("quantity")).collect()[0][0]
   write_log_info(v_procname,'test1 quantity '+str(test1),str(loadid),str(tid),provider_key)
   #print("------ Done generic_gras_Df >>>>>> ",generic_gras_Df)
   #exit()
   ...
   write_log_info(v_procname,' generic gras rules completed '+str(generic_gras_Df.count()),str(loadid),str(tid),provider_key)

   generic_gras_Df = generic_gras_Df.repartition((old_div(loaded_recs,part_recs_cnt))+1)
   generic_gras_Df.show(3,False)
   #print("----------- generic_gras_Df ----------")
   #exit()
   #Normal processing
   ...
   test2 = with_various_key_Df.agg(sum("quantity")).collect()[0][0]
   write_log_info(v_procname,'test2 quantity '+str(test2),str(loadid),str(tid),provider_key)
   ...
   write_log_info(v_procname,' with_various_key_Df loaded '+str(a_with_various_key_Df.count()),str(loadid),str(tid),provider_key)
   """
   ...
   #test3 = a_tmp.agg(sum("quantity")).collect()[0][0]
   #write_log_info(v_procname,'test3 quantity '+str(test3),str(loadid),str(tid),provider_key)
   ...
   test4 = with_scat_key_Df.agg(sum("quantity")).collect()[0][0]
   write_log_info(v_procname,'test4 quantity '+str(test4),str(loadid),str(tid),provider_key)


   a_with_scat_key_Df = with_scat_key_Df.alias('a_with_scat_key_Df')
   write_log_info(v_procname,' with_scat_key_Df loaded '+str(a_with_scat_key_Df.count()),str(loadid),str(tid),provider_key)
   ...
   write_log_info(v_procname,' df_with_ptypes loaded '+str(df_with_ptypes.count()),str(loadid),str(tid),provider_key)
   ...
   test5 = a_df_with_ptypes.agg(sum("quantity")).collect()[0][0]
   write_log_info(v_procname,'test5 quantity '+str(test5),str(loadid),str(tid),provider_key)
   ...
   write_log_info(v_procname,' df_with_consumer_key loaded '+str(df_with_consumer_key.count()),str(loadid),str(tid),provider_key)
   ...
   test6 = df_with_consumer_key.agg(sum("quantity")).collect()[0][0]
   write_log_info(v_procname,'test6 quantity '+str(test6),str(loadid),str(tid),provider_key)
   ...
   write_log_info(v_procname,' Persist done '+str(df_with_stgrp_key.count()),str(loadid),str(tid),provider_key)

   test7 = df_with_stgrp_key.agg(sum("quantity")).collect()[0][0]
   write_log_info(v_procname,'test7 quantity '+str(test7),str(loadid),str(tid),provider_key)
   ...
   write_log_info(v_procname,' df_with_stgrp_key loaded '+str(df_with_stgrp_key.count()),str(loadid),str(tid),provider_key)
   ...
   tmp1_test = df_with_stgrp_key.filter((col("Parent_Type_Id") == 1) & (col("Parent_Identifier").isNotNull())) 
   test7a = tmp1_test.agg(sum("quantity")).collect()[0][0]
   write_log_info(v_procname,'test7a quantity '+str(test7a),str(loadid),str(tid),provider_key)
   write_log_info(v_procname,'tmp1_test count '+str(tmp1_test.count()),str(loadid),str(tid),provider_key)

   tmp2_test = df_with_stgrp_key.filter(((col("Parent_Type_Id") == 17) | (col("Parent_Type_Id").isNull())) | ((col("Parent_Type_Id") == 1) & (col("Parent_Identifier").isNull())))
   test7b = tmp2_test.agg(sum("quantity")).collect()[0][0]
   write_log_info(v_procname,'test7b quantity '+str(test7b),str(loadid),str(tid),provider_key)
   write_log_info(v_procname,'tmp2_test count '+str(tmp2_test.count()),str(loadid),str(tid),provider_key)

   #IK
   print("tmp1 &temp 2")
   test8 = tmp1.agg(sum("quantity")).collect()[0][0]
   write_log_info(v_procname,'test8 quantity '+str(test8),str(loadid),str(tid),provider_key)
   write_log_info(v_procname,'tmp1 count '+str(tmp1.count()),str(loadid),str(tid),provider_key)

   test9 = tmp2.agg(sum("quantity")).collect()[0][0]
   write_log_info(v_procname,'test9 quantity '+str(test9),str(loadid),str(tid),provider_key)
   write_log_info(v_procname,'tmp2 count '+str(tmp2.count()),str(loadid),str(tid),provider_key)
   #df_with_stgrp_key.write.mode("overwrite").parquet("s3://sme-ca-dl-dev-output/partner_data/sme/P001/temp/c/")
   ...
   final_df = tmp1.union(tmp2).withColumnRenamed("prod_types_key","product_types_key")\
                                 .withColumn("media_key", when(col("product_identifier").isin({'U','W','X'}),'AP3').when(col("product_identifier").isin({'H','I','J'}),'MP3').otherwise('AAC')).distinct()
   final_df = final_df.repartition((old_div(loaded_recs,part_recs_cnt))+1)

   processed_recs = final_df.count()

   write_log_info(v_procname,' tmp1 and tmp2 loaded '+str(processed_recs),str(loadid),str(tid),provider_key)
   test10 = final_df.agg(sum("quantity")).collect()[0][0]
   write_log_info(v_procname,'test10 quantity '+str(test10),str(loadid),str(tid),provider_key)
   write_log_info(v_procname,'final_df count '+str(final_df.count()),str(loadid),str(tid),provider_key)
   ...
   cmd_string = 'final_df.write.mode("overwrite").format("csv").option("delimiter", "\x07").option("header","true").option("quote","").option("nullValue","").option("compression", "gzip")\
                                         .option("emptyValue",None).save("'+content_enriched_file_dir+licensor+'/'+provider_key+'/gras_enriched_files/report_date='+str(report_day_col)+'/DL_'+str(loadid)+'_enriched.csv.gz")'
   exec(cmd_string)
   ...
   success=upd_ins_data(connect(),'update '+config.metadata["trans_control"] +' set consumer_enriched_file_name = "'+'/gras_enriched_files/report_date='+str(report_day_col)+'/DL_'+str(loadid)+'_enriched.csv.gz"\
                                                                                 where load_id ='+str(loadid),'/gras_enriched_files/report_date='+str(report_day_col)+'/DL_'+str(loadid)+'_enriched.csv.gz"')

   write_manifest_file(loadid,'gras_enriched_files/DL_'+str(loadid)+'_enriched.csv.gz','enriched_files_dir_v2',clientKey)
...
def apply_generic_gras_match(in_df,a_gras_data_df,a_artist_data_df,a_labels_df,procargs):
   ...
   print("enrichedDataFinal >>>>>>>>>>>>>>> ",enrichedDataFinal.count())
   #exit()
...
def apply_special_rules(in_df,a_product_master,procargs):
   ...
   if step3_upd_recs > 0 or 1 == 1:
      ...
      a_prod_tmp = prod_tmp.alias('a_prod_tmp')
      step3_upd_recs = a_step2_Df.filter(col("UPC").isNull()).join(a_prod_tmp,(a_step1_Df.Parent_Identifier == a_prod_tmp.local_barcode),how='left')
   ...
   step10_Df.write.mode("overwrite").parquet(content_enriched_file_dir+licensor+'/P001/tmp_intermediate/report_day='+str(report_day_col)+'/Special_GRAS_DONE_'+str(loadid)+'_enriched.parquet')

   #a_step10_Df = step10_Df.alias('a_step10_Df')
   tmp_fname = content_enriched_file_dir+licensor+'/P001/tmp_intermediate/report_day='+str(report_day_col)+'/Special_GRAS_DONE_'+str(loadid)+'_enriched.parquet'
...
def proc_nc_appleitunes(procargs):
   ...
   consumer_Df = sqlContext.read.csv(content_enriched_file_dir+licensor+'/'+provider_key+'/gras_enriched_files/report_date='+str(report_day)+'/DL_'+str(loadid)+'_enriched.csv.gz',sep="\x07",header=True)
   #print cmd
   #exec(cmd)
   ...
   with_rep_week_Df.write.mode("overwrite").format("csv").option("delimiter", "\x07").option("header", "true").option("compression", "gzip").option("quote","").option("nullValue","") \
           .option("emptyValue",None).save(content_enriched_file_dir+"/"+licensor+"/"+provider_key+"/non_consumer_files/report_date="+report_day+"/DL_nc_"+str(loadid)+"_enriched.bsv.gz")
   ...
   upd_trans_stat(tid,'sum_units_db_nc','DL_nc_'+str(loadid)+'_enriched.bsv.gz')
...
def appleitunes_gras_main(load_id):
   ...
   if len(fileExpectation) == 0:
      ...
      #proc_appleitunes(fileExpectation)
      #exit()
      #trackFile = fileExpectation[0][12].lower()
      #trackFile = '/'.join(trackFile.split("/")[:9])+'/*.txt.gz'
