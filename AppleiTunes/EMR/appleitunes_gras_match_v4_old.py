...
sqlContext.sql("set spark.sql.broadcastTimeout=600")
...
def proc_appleitunes(procargs):
  ...
  #BY IK  exec('spcial_gras_Df = sqlContext.read.parquet("' + tmp_intermediate_fname +'/*")')
   ...
   write_log_info(v_procname,' before union tmp1 and tmp2 loaded '+str(tmp1.count())+ str(tmp2.count()),str(loadid),str(tid),provider_key)
