...
def itunes_load_partner_regions(load_id,lic,list_files):
    ...
    #print("====================>>>>>>>>>>>>>>>>>>> COUNT : ",tdf_new.count())
    #exit()
    #tdf_new=tdf_new.select("country_key", "zip_")
...
#               .withColumn("t_zip_code",when(length(col('zip_code')) == 0, -1).otherwise(col('zip_code')))
    ...
    strdf=strdf.join(pgdf,(lower(strdf.country_key)==lower(pgdf.country_code))& (lower(strdf.state_province)==lower(pgdf.state_province))& (lower(strdf.city)==lower(pgdf.city)),how='left').select("strdf.*","pgdf.partner_region_key").alias('strdf').na.fill({'country_key':'-1'})
